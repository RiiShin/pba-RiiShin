...
//        Debug.Log($"Volume: {volume_cog.Item1}, Center of Gravity: {volume_cog.Item2}");
    ...
    void Update()
    {
        for (int i = 0; i < 30; i++)
        {
            ...
            float3 domega = -math.mul(math.inverse(inertia_tensor), math.cross(omega, L));
    ...
    static float3x3 InertiaTensorForTriangleMesh(int[] tri2vtx, Vector3[] vtx2xyz) {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length/3; i_tri++) {
            ...
            float3x3 X = (OuterProduct(p0, p0) + OuterProduct(p1, p1) + OuterProduct(p2, p2)) / 10.0f;

            X += (OuterProduct(p0, p1) + OuterProduct(p1, p0)) / 20.0f;
            X += (OuterProduct(p0, p2) + OuterProduct(p2, p0)) / 20.0f;
            X += (OuterProduct(p1, p2) + OuterProduct(p2, p1)) / 20.0f;

            X *= volume;
            float tr = X.c0.x + X.c1.y + X.c2.z;
            res += tr * float3x3.identity - X;
